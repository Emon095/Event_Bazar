...
import { ArrowLeft, CalendarDays, CheckCircle2, Clock3, ExternalLink, Heart, MapPin, MessageCircle, Share2, ShieldCheck, Trophy, UsersRound } from "lucide-react";
...
export default async function EventDetails({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="details-page">
    ...
    <div className="detail-wrap">
      <div className={`detail-hero ${event.banner ? "" : "without-image"}`}>{event.banner && <Image src={event.banner} alt={`${event.title} event banner`} fill priority sizes="1200px"/>}<div className="detail-hero-shade"/><div className="detail-title"><span>{event.category} · {event.format}</span><h1>{event.title}</h1><p>Hosted by <b>{event.organizer}</b></p></div></div>
      <div className="detail-layout">
        <article className="detail-content">
          ...
          <section><h2>Community discussion</h2><div className="comment-box"><div className="avatar violet">AK</div><input placeholder="Ask a question or join the conversation…"/><button><MessageCircle/> Comment</button></div></section>
        </article>
        <aside className="detail-aside">
          ...
          <div className="interest-summary"><Heart/><div><b>{event.interested.toLocaleString()}</b><span>people are interested</span></div><Share2/></div>
        </aside>
      </div>
    </div>
  </main>;
